...
// Set default zoom to either Waterloo
...
function initialize_map(start, end) {
    ...
    for (incident in crime_data) {       
        ...
        if (crime_data[incident].date >= start && crime_data[incident].date <= end) {
            ...
            if (crime_data[incident].crime === 'HOMICIDE' && document.getElementById('homicide_box').checked === true) {
                ...
                crime_data_table += '<td>' + lowercase_letters(crime_data[incident].disposition) + '</td>';
                crime_data_table += '</tr>';
            } else if (crime_data[incident].crime === 'STABBING IN PROGRESS/JUST' && document.getElementById('weapons_violations').checked === true) {
                icon_url = 'weapons_icon';
                set_marker(lat, long, icon_url);
                weapons_violations_total += 1;
                // We'll add the crime information in the JSON file
                ...
                crime_data_table += '<td>' + lowercase_letters(crime_data[incident].disposition) + '</td>';
                crime_data_table += '</tr>';
            } else if (crime_data[incident].crime === 'ROBBERY IN PROGRESS/JUST' && document.getElementById('robbery').checked === true) {
                icon_url = 'robbery_icon';
                set_marker(lat, long, icon_url);
                robbery_total += 1;
                // We'll add the crime information in the JSON file
	...
	// We'll load a separate table if we're using a mobile phone or iPad
...
$(window).bind('enterBreakpoint480',function() {
	$(function() {
		...
		map = $('#map');
		console.log(map);
		// $('#map_table tbody').html('');
		// $('#map_table tbody').html('<tr>' + bar_chart + '</tr>');
		$('#map_location_mobile').append(map);
	});
});

$(window).bind('exitBreakpoint480',function() {
		map = $('#map');
		$('#map_location_mobile').html('');
		$('#map_location_div').append(map);
});
