...
var layers_mva = new L.MarkerClusterGroup({
    disableClusteringAtZoom: 15,
    maxClusterRadius: 60,
    typeofCrime: 'mva',
    polygonOptions: {
...
for (incident in crime_data) {
    // Some UNI reports don't have a time listed
    if (crime_data[incident].times === 'Unlisted') {
        crime_data[incident].times2 = 'Unlisted';
    } else {
...
function initialize_map(start, end) {
    ...
    function set_marker(lat, long, icon_url) {
        ...
        if (crime_data[incident].location_geo === 'UNI') {
            popup += "<br /><br />* Report provided by the <a href='http://www.vpaf.uni.edu/pubsaf/crime_stats/log/index.shtml' target='_blank'>UNI Police Department</a>";
        }

        popup += "</div>";
		layer.bindPopup(popup);
        
        if (crime_data[incident].crime === 'HOMICIDE') {
            layers_homicide.addLayer(layer);
        
        } else if (crime_data[incident].crime === 'SHOOTING IN PROGRESS/JUST') {
            ...
            layers_robbery.addLayer(layer);
        } else if (crime_data[incident].crime === 'Robbery - Assist') {
            layers_robbery.addLayer(layer);
        
        } else if (crime_data[incident].crime === 'ASSAULT') {
            ...
            layers_assault.addLayer(layer);
        } else if (crime_data[incident].crime === 'Assault - Assist') {
            layers_assault.addLayer(layer);
        } else if (crime_data[incident].crime === 'Sexual Assault') {
            layers_assault.addLayer(layer);
        } else if (crime_data[incident].crime === 'Sexual Assault - Assist') {
            layers_assault.addLayer(layer);
        } else if (crime_data[incident].crime === 'Sexual Assault/Burglary') {
            layers_assault.addLayer(layer);
        
        } else if (crime_data[incident].crime === 'MVA HIT & RUN') {
            layers_mva.addLayer(layer);
        
        } else if (crime_data[incident].crime === 'BURGLARY') {
            layers_burglary.addLayer(layer);
        } else if (crime_data[incident].crime === 'BURGLARY IN PROGRESS/JUST') {
            layers_burglary.addLayer(layer);
        } else if (crime_data[incident].crime === 'Burglary') {
            layers_burglary.addLayer(layer);
        } else if (crime_data[incident].crime === 'Burglary - Assist') {
            layers_burglary.addLayer(layer);
        }
