...
# Add headers
f.write("date" + "," + "time" "," + "crime" + "," + "location" + "," + "disposition" + "\n")
...
url = 'file:///Users/Essig/Desktop/crime_map/report/crpress_ind.html'
...
for x in range(71, 85):
    ...
    new_url = 'file:///Users/Essig/Desktop/crime_map/report/crpress-' + report_page + '.html'
    ...
    for post in new_soup.find_all(attrs={'style' : date_regex_css}):
        if date_regex.match(post.get_text()):
            if post.find_previous('div').get_text() == 'MVA HIT & RUN' or post.find_previous('div').get_text() == 'BURGLARY' or post.find_previous('div').get_text() == 'BURGLARY IN PROGRESS/JUST' or post.find_previous('div').get_text() == 'ROBBERY' or post.find_previous('div').get_text() == 'ASSAULT' or post.find_previous('div').get_text() == 'ASSAULT/AMBULANCE REQUESTED' or post.find_previous('div').get_text() == 'ASSAULT IN PROGRESS/JUST' or post.find_previous('div').get_text() == 'WEAPONS VIOLATIONS' or post.find_previous('div').get_text() == 'SHOOTING IN PROGRESS/JUST' or post.find_previous('div').get_text() == 'HOMICIDE' or post.find_previous('div').get_text() == 'WEAPON:SHOTS FIRED':
